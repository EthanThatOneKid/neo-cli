...
const errors: ClassifiedMessageMap = classifyMessageMap(constants.ERROR_TOKEN, {
  ...
  BAD_MAKE_PATTERN: (badIndex, gotNothing) => `Keyword 'make' expects a pattern of "TOY_NAME, typeName1, argName1, typeName2, argName2, ...", but at argument #${Number(badIndex) + 2}, it recieved ${gotNothing ? "nothing" : `a${Number(badIndex) % 2 === 0 ? "n argName" : " typeName"}`} instead of a${Number(badIndex) % 2 === 0 ? " typeName" : "n argName"}.`,
